...
jid1=`fsl_sub -q short.q -l ${outputdir}/logs1 /vols/Data/km/cetisca/projects/diffpostproc-exvivo-mouse-bruker7t/diffpostproc_step1.sh ${inputdir} ${shell1} ${shell2} ${blipDown} ${outputdir}`
echo Jobid $jid1
...
if [ ! -z $dtifit_opt ]
  then
    ...
    then
      ...
      jid8=`fsl_sub -q short.q -l ${outputdir}/logs5 -j $jid7 /vols/Data/km/cetisca/projects/diffpostproc-exvivo-mouse-bruker7t/diffpostproc_step5.sh ${outputdir} --dti_shell1`
    elif [[ $dtifit_opt == "dki_bothshells" ]]
    then
      echo Fitting FSL DKI model on both shells.
      jid8=`fsl_sub -q short.q -l ${outputdir}/logs5 -j $jid7 /vols/Data/km/cetisca/projects/diffpostproc-exvivo-mouse-bruker7t/diffpostproc_step5.sh ${outputdir} --dki_bothshells`
    fi
else
  echo Fitting FSLs DTI model on both shells.
  jid8=`fsl_sub -q short.q -l ${outputdir}/logs5 -j $jid7 /vols/Data/km/cetisca/projects/diffpostproc-exvivo-mouse-bruker7t/diffpostproc_step5.sh ${outputdir}`
fi
echo Jobid $jid8
